...
- ~~`reservations.guest_first_name` zasila „Witaj [Imię]" — seed musi go ustawić.~~ **Korekta (user, 2026-07-19):** roadmapa §9.3 jest tu nieaktualna — od s2-9 (minimalizacja PII) recepcja nie zbiera imienia, `guest_first_name = NULL`, welcome pokazuje „Witamy w pokoju {nr}" (`components/guest/welcome-banner.tsx:28`). Seed nie ustawia imienia; spec asertuje wariant z numerem pokoju.

...
export function BottomNav({ authLevel }: { authLevel: number }) {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 flex border-t border-guest-ink-muted/15 bg-guest-paper">
      {NAV_ITEMS.map((item) => {
        ...
        return (
          <Link
            ...
            className={`flex min-h-[44px] flex-1 flex-col items-center justify-center gap-1 px-1 py-2 text-xs font-medium ${
              isActive ? 'text-guest-accent' : 'text-guest-ink-muted hover:text-guest-ink'
            }`}
          >
            {t(item.labelKey)}
          </Link>

...
export default async function MyStayPage() {
  ...
  return (
    <main className="px-4 py-6">
      ...
      {guestFirstName && <p className="mt-4 text-base text-gray-900">{guestFirstName}</p>}

      {roomNumber && (
        <p className="mt-2 text-sm text-gray-600">
          {t('roomLabel')} <span className="font-mono text-gray-900">{roomNumber}</span>
        </p>
      )}
      ...
      {checkIn && checkOut && (
        <dl className="mt-4 space-y-1 text-sm text-gray-600">
          <div className="flex gap-2">
            ...
            <dd className="font-mono text-gray-900">{formatDate(checkIn, locale)}</dd>
          </div>
          <div className="flex gap-2">
            <dt>{t('checkOutLabel')}</dt>
            <dd className="font-mono text-gray-900">{formatDate(checkOut, locale)}</dd>
          </div>
        </dl>
      )}
